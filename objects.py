...
class Board:
    def __init__(self, height, width, food, hazards, snakes):
        ...
        self.snakes = [Snake(s['id'], s['name'], s['health'], s['body'], s['latency'], s['head'], s['length'], s['shout'], s['squad'], s['customizations']) for s in snakes]
        ...
        for y in range(self.height):
            ...
            for x in range(self.width):
                if Coordinate(x, y) in self.food:
                    ...
                    snake = next(snake for snake in self.snakes if snake.is_head_at(Coordinate(x, y)))
                    if snake.id == "you":
                        row_str += "Y"
                    else:
                        row_str += "S"
...
class Snake:
    ...
    def __init__(self, id, name, health, body, latency, head, length, shout, squad, customizations):
        ...
        self.shout = shout
        self.squad = squad
        self.customizations = self.Customizations(customizations['color'], customizations['head'], customizations['tail'])
